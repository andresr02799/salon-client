                ...
                <tr key={item.id}>
                  ...
                  <td class='border'>
                    <button onClick={() => editItem(item)}>
                      <i class="bi bi-pencil-square text-2xl hover:text-midnight hover:text-xl"></i>
                    </button>
                  </td>
                  <td class='border'>
                    <button onClick={() => setModalOpen(item.id)}>
                      <i class="bi bi-trash text-2xl hover:text-redUnicauca hover:text-xl"></i>
                    </button>
                  </td>
                  {modalOpen === item.id && (
                    createPortal(
                      <Modals
                        closeModal={() => handleButtonClick(item.id)}
                        onCancel={() => handleButtonClick(null)}
                        leyenda='Estas seguro(a) de eliminar este salon?'
                        instruccion='Eliminar'
                        icono='bi bi-trash text-5xl text-redUnicauca'
                        clickBotonPrincipal={() => deleteItem(item.id)}
                      />,
                      document.body
                    )
                  )}
